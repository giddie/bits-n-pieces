...
# The script presents the user with a temporary file in the default editor (set @EDITOR@; Vim is the default).  The file contains a list of actions performed by Pacman, grouped into actions that were perfored together.
# The user deletes all lines except those describing the actions he wants to roll back.  In Vim, it is particularly easy to delete the large number of unwanted lines using @SHIFT-V@ to enter __Visual-Line__ mode, then @CTRL+G@ to skip to the end of the file, and finally @d@ to delete all the highlighted lines.
